...
import com.yibo.saiku.olap.discover.OlapMetaExplorer;
...
public class OlapDiscoverService implements Serializable {
  ...
  public Map<String, Object> getProperties(SaikuCube cube) {
    ...
    try {
      ...
      properties.put("com.yibo.saiku.query.explain", con.isWrapperFor(RolapConnection.class));

      try {
        Boolean isScenario = (c.getDimensions().get("Scenario") != null);
        properties.put("com.yibo.saiku.connection.scenario", isScenario);
      } catch (Exception e) {
        properties.put("com.yibo.saiku.connection.scenario", false);
      }
    } catch (Exception e) {
      throw new SaikuServiceException(e);
    }
