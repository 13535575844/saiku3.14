...
import com.yibo.saiku.query.Query.BackendFlavor;
 ...
 */
public class Olap4jNodeConverter extends com.yibo.saiku.query.NodeConverter {
	...
	public static SelectNode toQuery(Query query) throws Exception {
		...
		List<com.yibo.saiku.query.QueryAxis> axisList = new ArrayList<com.yibo.saiku.query.QueryAxis>();
		...
		if (query.getAxes().containsKey(Axis.FILTER)) {
			final com.yibo.saiku.query.QueryAxis axis = query.getAxes().get(Axis.FILTER);
	...
	private static List<AxisNode> toAxisList(List<ParseTreeNode> withList, List<com.yibo.saiku.query.QueryAxis> axes) throws Exception {
		final ArrayList<AxisNode> axisList = new ArrayList<AxisNode>();
		for (com.yibo.saiku.query.QueryAxis axis : axes) {
	...
	private static AxisNode toAxis(List<ParseTreeNode> withList, QueryAxis axis) throws Exception {
		...
		if (!axis.isMdxSetExpression()) {
			...
			if (hierarchyCount == 1) {
				...
				for(com.yibo.saiku.query.QueryHierarchy h : axis.getQueryHierarchies()) {
...
//		TODO - it seems like its better to have the crossjoin as close to the NON EMPTY axis etc. as possible in mondrian 3 - works ok in mondrian 4
		...
		com.yibo.saiku.query.QueryDetails details = axis.getQuery().getDetails();
	...
	private static ParseTreeNode toHierarchy(List<ParseTreeNode> withList, QueryHierarchy h) throws OlapException {
		...
		if (!h.isMdxSetExpression()) {
			...
			for (int i = 0; i < h.getActiveQueryLevels().size(); i++) {
				com.yibo.saiku.query.QueryLevel l = h.getActiveQueryLevels().get(i);
...
//		}
	...
	private static ParseTreeNode toSortedQuerySet(ParseTreeNode expression, com.yibo.saiku.query.ISortableQuerySet o) {
		...
		if (o.getSortOrder() != null) {
			...
			if (o.getHierarchizeMode().equals(
					com.yibo.saiku.query.ISortableQuerySet.HierarchizeMode.PRE))
			{
